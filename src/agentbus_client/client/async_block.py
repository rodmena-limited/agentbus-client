"""Blocking a peer whose mail you no longer want — async twin (#48).
...
Built in the SAME change as the sync mixin, not after it. This pair has drifted
before — `phonebook(label=)` landed on one and not the other, and async `read`
once skipped unsealing entirely — and a twin added later is a twin that differs.
"""
...
class AsyncBlockMixin(_MixinBase):
    async def block(
        self,
        agent_name: str,
        *,
        ...
        return await self._request("POST", "/v1/blocks", json=payload, agent=agent)

    async def unblock(self, agent_name: str, *, agent: str | None = None) -> dict[str, Any]:
        """Resume delivery from `agent_name`."""
        return await self._request("DELETE", f"/v1/blocks/{agent_name}", agent=agent)

    async def blocks(self, *, agent: str | None = None) -> list[dict[str, Any]]:
        ...
        """
        data = await self._request("GET", "/v1/blocks", agent=agent)
